  ...
  implements OnGatewayConnection, OnGatewayDisconnect
{
  ...
  handleConnection(client: Socket) {
    console.log(
      `[WhiteboardGateway] Client connected: ${client.id}, Origin: ${client.handshake.headers.origin}`,
    );
    this.logger.log(
      `Client connected: ${client.id}, Origin: ${client.handshake.headers.origin}`,
    );
  }
  ...
  handleJoin(
    ...
    console.log(
      `[WhiteboardGateway] Client ${client.id} joining room ${data.room}`,
    );
  ...
  handleCursor(@MessageBody() data: any, @ConnectedSocket() client: Socket) {
  ...
  handleDrawBatch(@MessageBody() data: any, @ConnectedSocket() client: Socket) {
    const room = Array.from(client.rooms).find((r) => r !== client.id);
    if (room) {
  ...
  async handleAddItem(
    ...
    this.logger.debug(
      `[Gateway] Received add_item for ${data?.id} from ${client.id}`,
    );
    // Log the meetingId specifically
    if (!data.meetingId) {
      console.warn(
        `[Gateway] Item ${data?.id} missing meetingId! Defaulting to 'default'`,
      );
      this.logger.warn(
        `[Gateway] Item ${data?.id} missing meetingId! Defaulting to 'default'`,
      );
  ...
  handleRefetch(@MessageBody() data: any, @ConnectedSocket() client: Socket) {
    const room = Array.from(client.rooms).find((r) => r !== client.id);
    if (room) {
  ...
  handleStrokeEnd(@MessageBody() data: any, @ConnectedSocket() client: Socket) {
    const room = Array.from(client.rooms).find((r) => r !== client.id);
    if (room) {
