...
import {
  CreateWhiteboardItemDto,
  UpdateWhiteboardItemDto,
  UndoRequestDto,
} from './dto/whiteboard-item.dto';
...
export class WhiteboardController {
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          ...
          if (/\.(jpg|jpeg|png|gif|webp)$/i.test(entry.name)) {
            const relPath = relative(process.cwd(), fullPath).replace(
              /\\/g,
              '/',
            );
    ...
    FileInterceptor('file', {
      storage: diskStorage({
        destination: (req, file, cb) => {
          ...
          const meetingName =
            rawName.replace(/[^a-zA-Z0-9가-힣_\- ]/g, '').trim() || 'Untitled';
  ...
  @Post()
  create(@Body() createWhiteboardItemDto: CreateWhiteboardItemDto) {
  ...
  async findAll(@Param('meetingId') meetingId: string) {
    console.log(
      `[WhiteboardController] GET /:meetingId hit with ID: ${meetingId}`,
    );
