...
async function run() {
    ...
    try {
        ...
        await dataSource.query('ALTER TABLE workspaces ADD COLUMN IF NOT EXISTS thumbnail text');

        console.log('Adding banner column...');
        await dataSource.query('ALTER TABLE workspaces ADD COLUMN IF NOT EXISTS banner text');
