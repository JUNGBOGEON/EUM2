...
export class ChatGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  handleJoinChannel(
    ...
    client.join(room);
    this.logger.log(
      `[DEBUG] Client ${client.id} joined ${room}. Data: ${JSON.stringify(data)}`,
    );
  }
  ...
  async handleSendMessage(
    @MessageBody()
    data: { channelId: string; content: string; senderId: string },
    @ConnectedSocket() client: Socket,
  ) {
    this.logger.log(
      `[DEBUG] Received message from ${client.id}: ${JSON.stringify(data)}`,
    );
    try {
      ...
      if (!hasPermission) {
        this.logger.warn(
          `User ${data.senderId} does not have sendMessages permission in workspace ${channel.workspaceId}`,
        );
        client.emit('error', {
      ...
      // 4. Broadcast to room
      const fullMessage = await this.chatService.getMessageWithSender(
        savedMessage.id,
      );
      const room = `channel:${data.channelId}`;
      this.logger.log(
        `[DEBUG] Broadcasting to room ${room}: ${JSON.stringify(fullMessage)}`,
      );

      this.server.to(room).emit('new_message', fullMessage);
    } catch (error) {
      this.logger.error(
        `[DEBUG] Error sending message: ${error.message} - Stack: ${error.stack}`,
      );
